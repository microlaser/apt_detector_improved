/*
 * macos_apt_detector_v15.c
 * macOS APT Detector — v15.0
 *
 * New in v15:
 ...
 */
/* proc_signed: 1 if the process binary carries any valid signature, 0 if not.
 * Anonymous exec regions only scored for UNSIGNED processes — signed binaries
 * (Homebrew, Steam, etc.) produce them via dyld trampolines/JIT. RWX always scored.
 */
static int scan_process_memory(pid_t pid, const char *proc_name, int proc_signed) {
    int risk = 0;
    ...
    while (1) {
        ...
        if (is_rwx) {
            ...
            if (!proc_signed) {
                /* Only flag for unsigned processes — signed binaries produce
                 * these legitimately via dyld trampolines and JIT. */
                printf(COL_YELLOW
                       "  [?] Anon exec region, UNSIGNED process PID %-6d  addr=0x%llx  %s (+15)\n"
...
int run_memory_scan(void) {
    ...
    for (int i = 0; i < proc_count; i++) {
        ...
        if (pid == getpid()) continue;   /* skip self — IOC patterns in our own
                                            data segment would always match */
        ...
        int proc_signed = binary_is_signed(proc_name);
        printf(" [PID %-6d] %-8s  %s\n", pid,
               proc_signed == 1 ? "[signed]" : (proc_signed == 0 ? "[UNSIGNED]" : "[?]"),
               proc_name);
        int r = scan_process_memory(pid, proc_name, proc_signed);
        if (r > 0) total_risk += r;
        scanned++;
    }
...
void report_final_score(ScanReport *r) {
    ...
    printf("\n" COL_WHITE
           "╔══════════════════════════════════════════════════════════╗\n"
           "║       macOS APT Detector v15.0 — FINAL REPORT           ║\n"
           "╠══════════════════════════════════════════════════════════╣\n" COL_RESET);
...
int main(void) {
    if (getuid() != 0) {
        fprintf(stderr, "Error: must be run as root (sudo ./macos_apt_detector_v15)\n");
        return 1;
    }
